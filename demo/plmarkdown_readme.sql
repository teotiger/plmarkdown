...
BEGIN
  ...
  l_md.ul('`h1(p_val VARCHAR2)`, `h2(p_val VARCHAR2)`, `h3(p_val VARCHAR2)`, `h4(p_val VARCHAR2)`, `h5(p_val VARCHAR2)` and `h6(p_val VARCHAR2)`<br>A header text.');
  ...
  l_md.ul('`sql2table(p_sql_statement VARCHAR2, p_null_display VARCHAR2 DEFAULT ''--'', p_date_format VARCHAR2 DEFAULT ''dd.mm.yyyy'', p_number_format VARCHAR2 DEFAULT ''FM9G999G999G999G990D00'')`<br>Convert a sql statement to a table.');
  l_md.ul('`sql2chart(p_sql_statement VARCHAR2, p_chart_type VARCHAR2, p_show_legend BOOLEAN DEFAULT FALSE, p_image_type VARCHAR2 DEFAULT ''png'')`<br>Convert a sql statement to a chart.');
  l_md.p('**Notes:** Valid chart types are ''pie'', ''bar'', ''line'' and ''area'', valid image types ''png'' and ''svg''. For more information see the [plmarkdown_chartexample.sql](https://github.com/teotiger/plmarkdown/blob/master/demo/plmarkdown_chartexample.sql) in the demo section or visit the website from [chartspree.io](http://chartspree.io).');
  ...
  l_md.p('Version 1.3 - July 7, 2016');
  l_md.ul('sql2chart added');
  l_md.p('');
