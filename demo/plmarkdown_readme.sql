...
BEGIN
  ...
  l_md.ul('`sql2chart(p_sql_statement VARCHAR2, p_chart_type VARCHAR2, p_show_legend BOOLEAN DEFAULT FALSE, p_image_type VARCHAR2 DEFAULT ''png'')`<br>Convert a sql statement to a chart.<br>**Notes:** Valid chart types are ''pie'', ''bar'', ''line'' and ''area'', valid image types ''png'' and ''svg''. For more information see [plmarkdown_hr_schema.md](https://github.com/teotiger/plmarkdown/blob/master/demo/plmarkdown_hr_schema.md) in the demo section or visit the website from [chartspree.io](http://chartspree.io).');
